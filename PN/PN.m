...
Vt0 = [-180; 0;  50];   % Shahed initial velocity  [m/s], head-on + slight descent
...
[tm, tt, t_h, Omega_h, Vc_h, R_h, acmd_h, miss, hit] = ...
    Sim3DPN(Rm0, Vm0, Rt0, Vt0, N, Ts, Tmax, Rmin, Amax);
...
while t < Tmax
    ...
    % LOS angular rate magnitude  (3D equivalent of lambda_dot in 2D)
    omega_mag = norm(Omega);

    % Missile velocity unit vector
    Vm_hat = Vm / norm(Vm);

    % Acceleration direction: perpendicular to missile velocity
    a_dir = cross(Omega, Vm_hat);
    if norm(a_dir) > 1e-10
        a_dir = a_dir / norm(a_dir);
    end

    % Basic PN guidance law:  a_c = N * Vc * |Omega|
    a_cmd = N * Vc * omega_mag * a_dir;
...
title(sprintf('Basic PN 3D  (N=%d)  |  R=%.0fm  |  t=%.2fs', N, R_h(k), t_h(k)), 'FontSize', 11)
...
title('$|a_{c}| = N \cdot V_c \cdot |\dot{\lambda}|$', 'Interpreter', 'latex')
xlim([0 t_h(end)])

sgtitle(sprintf('Basic PN Guidance  $|$  $N = %d$', N), ...
