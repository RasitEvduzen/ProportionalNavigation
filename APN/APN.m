...
% Augmented Proportional Navigation - 3D APN
% Guidance: a = N * Vc * (Omega x r_hat) + N/2 * at_perp
...
%% Target Maneuver (Shahed evasive)
Am = 30;     % maneuver amplitude [m/s^2]  (~3g)
fm = 0.3;    % maneuver frequency [Hz]
...
[tm, tt, t_h, Omega_h, Vc_h, R_h, acmd_h, miss, hit] = ...
    Sim3DAPN(Rm0, Vm0, Rt0, Vt0, N, Ts, Tmax, Rmin, Amax, Am, fm);
...
while t < Tmax
    ...
    % Target maneuver acceleration (sinusoidal evasive)
    at = [0; Am * sin(2*pi*fm*t); Am * 0.5 * cos(2*pi*fm*t)];

    % Target accel perpendicular to LOS (parallel component does not affect ZEM)
    at_perp = at - (at' * r_hat) * r_hat;

    % APN guidance law: a = N * Vc * (Omega x r_hat) + N/2 * at_perp
    % TPN base term corrects LOS rate, feed-forward term anticipates target maneuver
    a_cmd = N * Vc * cross(Omega, r_hat) + (N/2) * at_perp;
...
title(sprintf('APN 3D  (N=%d)  |  R=%.0fm  |  t=%.2fs', N, R_h(k), t_h(k)), 'FontSize', 11)
...
title('$\mathbf{a} = N V_c (\Omega \times \hat{r}) + \frac{N}{2} \mathbf{a}_{t\perp}$', 'Interpreter', 'latex')
xlim([0 t_h(end)])

sgtitle(sprintf('Augmented Proportional Navigation  $|$  $N = %d$', N), ...
