...
% Pure Proportional Navigation - 3D PPN
% Guidance: a = N * Vc * (Omega x Vm_hat)
...
[tm, tt, t_h, Omega_h, Vc_h, R_h, acmd_h, miss, hit] = ...
    Sim3DPPN(Rm0, Vm0, Rt0, Vt0, N, Ts, Tmax, Rmin, Amax);
...
while t < Tmax
    ...
    % PPN guidance law: a = N * Vc * (Omega x Vm_hat)
    % cross(Omega, Vm_hat) encodes both direction and sin(theta) factor
    % as missile aligns with LOS: sin(theta)->0, command naturally reduces
    a_cmd = N * Vc * cross(Omega, Vm_hat);
...
title(sprintf('PPN 3D  (N=%d)  |  R=%.0fm  |  t=%.2fs', N, R_h(k), t_h(k)), 'FontSize', 11)
...
title('$\mathbf{a} = N \cdot V_c \cdot (\Omega \times \hat{V}_m)$', 'Interpreter', 'latex')
xlim([0 t_h(end)])

sgtitle(sprintf('Pure Proportional Navigation  $|$  $N = %d$', N), ...
